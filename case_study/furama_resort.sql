...
CREATE PROCEDURE add_facility
	(in `name` varchar(55), dien_tich_moi int, gia double, nguoi int, phong varchar(55), tien_nghi varchar(55), ho_boi double,tang int, mien_phi varchar(100), kieu_thue int, loai_dich_vu int)
BEGIN
...
BEGIN
   update dich_vu 
		set 
			...
			chi_phi_thue = gia, 
			so_nguoi_toi_da = nguoi,
			tieu_chuan_phong = phong, 
			mo_ta_tien_nghi_khac = tien_nghi, 
			dien_tich_ho_boi = ho_boi, 
			so_tang = tang,
			dich_vu_mien_phi_di_kem = mien_phi,
			ma_kieu_thue = kieu_thue, 
			ma_loai_dich_vu = loai_dich_vu 
        where ma_dich_vu = id;
END $$
DELIMITER ;
